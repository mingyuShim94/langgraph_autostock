...
- [ ] `TradeRecord` 테이블에 에이전트별 기여도 필드 추가
- [ ] `AgentPerformance` 테이블 신규 생성 (에이전트별 성과 추적)
- [ ] `LLMUsageLog` 테이블 신규 생성 (모델별 사용량 및 비용)
- [ ] `ModelEvolutionHistory` 테이블 신규 생성 (모델 교체 이력)
- [ ] 데이터베이스 마이그레이션 스크립트 작성 및 테스트

#### 🔍 성과 분석 및 기여도 측정 시스템

- [ ] 에이전트별 의사결정 기여도 계산 알고리즘
- [ ] 거래 성과와 에이전트 분석의 상관관계 추적
- [ ] LLM 모델별 성과 벤치마킹 데이터 수집
