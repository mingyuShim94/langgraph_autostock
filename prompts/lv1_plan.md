...
- [ ] requirements.txt 작성
- [ ] LangGraph, LangChain 핵심 패키지 설치
- [ ] 추가 필요 패키지 설치 (requests, python-dotenv, pandas)

### 1.2 프로젝트 구조 생성
- [ ] 프로젝트 디렉토리 구조 설계
- [ ] `src/` 디렉토리 생성 (메인 코드)
- [ ] `config/` 디렉토리 생성 (설정 파일)
- [ ] `tests/` 디렉토리 생성 (테스트 코드)
- [ ] `logs/` 디렉토리 생성 (로그 파일)

### 1.3 기본 설정 파일 작성
- [ ] `.env` 파일 생성 (API 키 등 환경변수)
- [ ] `config/settings.py` 작성 (기본 설정)
- [ ] `.gitignore` 업데이트 (환경변수, 로그 파일 제외)

### 1.4 API 연동 준비
- [ ] 한국투자증권 KIS API 계정 생성 (또는 모의투자)
- [ ] API 키 발급 및 환경변수 설정
- [ ] 뉴스 API 선택 및 설정 (네이버 뉴스 API 등)
- [ ] API 연결 테스트
...
### 2.1 State 스키마 정의
- [ ] `State` 클래스 정의 (TypedDict 활용)
- [ ] `portfolio_status` 필드 설계 (종목코드, 수량, 평가금액 등)
- [ ] `news_data` 필드 설계 (종목별 뉴스 리스트)
- [ ] `daily_report` 필드 설계 (최종 브리핑 내용)
- [ ] 기타 메타데이터 필드 추가 (timestamp, status 등)
...
### 2.3 에러 핸들링 구조 설계
- [ ] 각 노드별 예외 처리 전략 수립
- [ ] 로깅 시스템 설계 (logging 모듈 활용)
- [ ] API 호출 실패 시 재시도 로직 설계
- [ ] 그래프 실행 중단 시 복구 방안 설계
